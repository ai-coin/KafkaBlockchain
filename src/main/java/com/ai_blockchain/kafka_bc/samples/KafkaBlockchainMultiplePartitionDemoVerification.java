...
public class KafkaBlockchainMultiplePartitionDemoVerification {
  ...
  private static final String KAFKA_GROUP_ID = "multiple-partition-demo-blockchain-consumers";
  ...
  public static void main(final String[] args) {
    if (args != null && args.length > 0 && args[0] != null) {
      ...
      blockchainName = "kafka-demo-multiple-partition-blockchain";
  ...
  public void verifyDemoBlockchain() {
    ...
    final List<TopicPartition> assignedTopicPartitions = KafkaUtils.getNbrPartitionsForTopic(
            blockchainName,
            consumerProperties);
    ...
    for (int i = 0; i < nbrPartitions; i++) {
      ...
      final Thread thread = new Thread(kafkaBlockchainPartitionConsumer);
    ...
    LOGGER.warn("finding the genesis record for " + blockchainName + "...");
    ...
    for (int i = 0; i < nbrPartitions; i++) {
      ...
      if (genesisSHA256Hash.equals(teObject.getTEObjectHash())) {
        ...
        final Object lock = kafkaBlockchainPartitionConsumer.getLock();
        synchronized (lock) {
          lock.notify();
        }
        break;
    ...
    while (isStillRunning) {
      ...
      for (int i = 0; i < nbrPartitions; i++) {
        ...
        final TEObject teObject = kafkaBlockchainPartitionConsumer.getTeObject();
        if (teObject == null) {
          // this partition consumer is still reading
          continue;
        }
        if (teObject.getSerialNbr() == nextSerialNbr) {
          LOGGER.info("found successor " + teObject);
   ...
   */
  static class KafkaBlockchainPartitionConsumer implements Runnable {
    ...
    // the topic (blockchain name)
    private final String topic;
    // the topics, which has only one topic, which is the the blockchain name
    private final List<String> topics = new ArrayList<>();
    // the lock used to suspend the consumer until its current block is sequentially ordered and verified
    ...
    private transient TEObject teObject = null;
        ...
        while (isStillRunning) {
          ...
          for (ConsumerRecord<String, byte[]> consumerRecord : consumerRecords) {
            if (++recordCnt % 100000 == 0) {
              LOGGER.warn("thread " + partition + " read " + recordCnt + " records from its partition");
            }
            LOGGER.info("thread " + partition + " received consumerRecord " + consumerRecord);
            ...
            // wait for the main thread to use up the current tamper evident object before examining the next one from this partition
            synchronized (lock) {
              try {
                lock.wait();
    ...
    /**
     * Gets the lock used to suspend the consumer until its current block is sequentially ordered and verified.
     *
     * @return the lock used to suspend the consumer until its current block is sequentially ordered and verified
     */
